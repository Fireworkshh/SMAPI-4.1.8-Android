    ...
    *********/
    ...
    public IEnumerable<IInstructionHandler> GetHandlers(bool paranoidMode, bool rewriteMods, bool logTechnicalDetailsForBrokenMods)
    {
        ...
        ****/
        ...
        if (rewriteMods)
        {
           ...
           //     .MapType("StardewValley.AudioEngineWrapper", typeof(AudioEngineWrapper))
               ...
               //666 .MapType("StardewValley.DummyAudioEngine", typeof(DummyAudioEngine))
        ...
        ****/
        ...
        Type saveGameType = typeof(SaveGame);

        // 字段名称数组
        string[] fieldNames = { "serializer", "farmerSerializer", "locationSerializer" };

        // 使用Harmony访问字段
        foreach (string fieldName in fieldNames)
        {
            // 获取私有字段
            FieldInfo field = AccessTools.Field(saveGameType, fieldName);
            if (field != null)
            {
                // 假设我们用一个方法获取 SaveGame 实例
                SaveGame saveGameInstance = SaveGame.loaded;

                // 获取字段的值
                object? fieldValue = field.GetValue(saveGameInstance);

                // 使用yield返回FieldFinder实例
                yield return new FieldFinder(
              saveGameType.FullName!,
              new[] { fieldName },
              InstructionHandleResult.DetectedSaveSerializer // 使用正确的枚举值
          );
            }
